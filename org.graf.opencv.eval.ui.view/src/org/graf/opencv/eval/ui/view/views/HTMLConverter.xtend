...
class HTMLConverter {
	def dispatch conv(Object arg) '''
	«arg»
	'''
	
	def dispatch conv(Mat arg) '''
		«IF arg.width < 5 && arg.height < 5»
			«arg.m2h»
		«ELSE»
	...
	def dispatch CharSequence conv(List<Object> arg) '''
		«IF arg.forall[it instanceof Mat]»
			<table><tr>«FOR m:arg.filter(Mat)»<td>«this.conv(m)»</td>«ENDFOR»</tr></table>
		«ELSE»
			«arg»
		«ENDIF»
	'''
